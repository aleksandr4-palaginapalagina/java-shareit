...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("Select distinct b From Booking AS b join b.item AS i Where b.id = ?1 and (b.booker.id = ?2 or i.owner.id = ?2)")
    ...
    @Query("Select b From Booking AS b join b.item AS i Where i.owner.id = ?1 and b.start < ?2 and b.end > ?2 ORDER BY b.start DESC")
    ...
    @Query("Select b From Booking AS b Where b.item.id = ?1 and (b.start >= ?2 and b.end <= ?3)")
